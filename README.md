...
5. On a new terminal, while still in the project directory, call the service client. 
  ...
  - `data`: csv file with observations or samples in rows and binary features in columns labeled *1* for *TRUE* and *0* for *FALSE* for each sample.  The first column should indicate the category label of the sample (*1* for *case* and *0* for *control*)  See the doc [here](https://habush.github.io/moses-service/users_guide/moses-service.html) for a discussion of prepairing specific experimental data types.
